  ...
  for( i in 1:length(files) ) { 
    ...
    for( j in 1:length(files) ) { 
      ...
      if( i < j ) { 
        ...
        fit <- test_congruence(g,g1)$statistic
        if(is.na(fit)) { 
          
          
          
          cat("na value in congruence", i,j,"\n")
          print(files[i])
          print(files[j])
          stop()
        }
        A[i,j] <- test_congruence(g,g1)$statistic
      }
    }
  }
  cat("\n")
...
df_congruence %>%
  ggplot( aes(as.factor(NumLoci), Congruence) ) + 
  geom_boxplot( notch=TRUE ) + 
  xlab("Number of SNP Loci") + 
  ylab("Varince in Structural Congruence")
```
...
varCong %>%
  ...
  geom_point() + 
  stat_smooth(se=FALSE, method="nls", formula = y ~ SSasymp(x,Asym,R0,lrc), linetype=2 ) + 
  xlab("Number of SNP Loci") + 
  ylab("Varince in Structural Congruence")
```
